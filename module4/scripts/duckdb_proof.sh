...
# ─────────────────────────────────────────────────────────────────────────────
# Module 4 — DuckDB CLI proof moment (one screen, ~10 seconds on camera)
#
# Reads the live northwind.duckdb file (via a read-only snapshot so FastAPI's
# writer keeps its lock) and prints, on one screen:
#   - trusted.feedback_enriched   (sample accepted row + total count)
#   - quarantine.llm_outputs      (sample rejected row + reason + total)
#
# Same brand palette as Step 3 so the recap looks native to the clip.
# ─────────────────────────────────────────────────────────────────────────────

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
SOURCE_DB="${DUCKDB_PATH:-$PROJECT_ROOT/data/northwind.duckdb}"

SNAPSHOT_DIR="$(mktemp -d -t duckdb_proof_m4_XXXXXX)"
...
section(
    ...
    LIME,
    "accepted rows the validation gate let through",
    ["request_id", "category", "confidence", "validation_status"],
)
section(
    "quarantine.llm_outputs",
    PINK,
    "rejected rows with the validation reason that caught them",
    ["request_id", "validation_errors"],
)

print()
print(f"  {GRAY}Two distinct DuckDB schemas — the disposition outcome the validation gate enforced.{NC}")
print()
PY
